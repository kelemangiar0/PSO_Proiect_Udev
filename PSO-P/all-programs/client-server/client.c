...
int main(int argc, char**argv) {
    char ip[16];
    strcpy(ip, argv[2]); // adresa IP a serverului
    int port = 8084;
    int socket = setup_connection(ip, port);

    char source_folder[256]; // Allocate enough space for the path
    sprintf(source_folder, "/home/%s/PSO-P/backup-folder", argv[1]); // folderul pe care il vrei sa il trimiti catre server (de la client)
    ...
    char data1[SIZE] = {0};
    strcpy(data1, argv[1]);
    if (send(socket, data1, sizeof(data1), 0) == -1) {
        perror("[-] Error in sending data");
        exit(1);
    }
    sleep(1);
